/* Code from ResForge under MIT license, Copyright (c) 2024 Andrew Simmonds. */
...
enum PictOpcode: UInt16 {
    ...
    static func read2(_ reader: BinaryDataReader) throws -> Self {
		...
		let rawValue: UInt16 = try reader.read()
        guard let op = Self.init(rawValue: rawValue) else {
			print("Unknown opcode \(rawValue)")
            throw ImageReaderError.unsupported
        }
        return op
    }
    ...
    static func read1(_ reader: BinaryDataReader) throws -> Self {
		let rawValue = UInt16(try reader.read() as UInt8)
        guard let op = Self.init(rawValue: rawValue) else {
			print("Unknown opcode \(rawValue)")
            throw ImageReaderError.unsupported
        }
